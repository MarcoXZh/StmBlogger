...
let runJob = function(options, callback) {
  ...
  let query = ('SELECT * FROM Comments ' +        // the target table
 ...
 * @param {function}  callback  (optional) the callback function
 */
let analyzeData = function(options, records, callback) {
  let tags = {};
  // let categories = {};
 ...
 * @param {function}  callback  (optional) the callback function
 */
let statistics = function(options, results, callback) {
  let tags  = results.tags;
  let keys = Object.keys(tags);
  console.log('QueryCnTags: found - tags=' + keys.length);

  let stats = {count: keys.length};
  ...
  stats.tag_pay_avg =
    sort(keys.map( (k)=>(10000*tags[k].total_payout_value/tags[k].count)+
                         '/'+k ))
    .map((e)=>e.split('/') );
...
/*
  ...
  stats.cat_pay_avg =
    sort(keys.map( (k)=>(10000*categories[k].total_payout_value/
                         categories[k].count)+'/'+k ))
    .map((e)=>e.split('/') );
*/

  prepareBlog(options, stats, callback);
}; // let statistics = function(options, result, callback) { ... };
 ...
 * @param {function}  callback  (optional) the callback function
 */
let prepareBlog = function(options, stats, callback) {
  ...
  let half = Math.ceil(0.5 * options.count);
...
let publishBlog = function(options, blog, callback) {
  ...
  let permlink = options.author.name + '-cn-tag-analysis-' +
