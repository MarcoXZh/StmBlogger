...
var sql1 = `SELECT * FROM INFORMATION_SCHEMA.TABLES`;
// Query for columns in table
var sql2 = `SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'...'`;
var texts = '# ' +  sql1 + '\n#     ' + sql2 + '\n\n';
...
var queryTable = function(idx, tables, callback) {
    var name = tables[idx].TABLE_NAME;
    texts += (tables.length>9&&idx<9?' ':'') + (idx+1) + '/' + tables.length + ' '
                                 + name;
    ...
    new sql.Request().query(sql2.replace('...', name), function(err, res) {
        ...
        var max = Math.max.apply(null, Object.values(res.recordset).map( (e)=>e.COLUMN_NAME.length ));
        // Check each column
        res.recordset.forEach(function(e, idx1, arr) {
            var col = e.COLUMN_NAME;
            while (col.length < max) {
                col += ' ';
            } // while (col.length < max)
            texts += '    ' + (arr.length>9&&idx1<9?' ':'') + (idx1+1) + '/' + arr.length + ' '
                            + col + ' ' + e.DATA_TYPE + '\n';
